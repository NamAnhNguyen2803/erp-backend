...
exports.getAllMaterials = async (req, res) => {
  try {
    const { page = 1, limit = 10, status, factory } = req.query;
    ...
    const { count, rows } = await Material.findAndCountAll({
      ...
      attributes: ['material_id', 'code', 'name', 'unit', 'factory', 'current_stock', 'unit_price', 'status'],
...
exports.createMaterial = async (req, res) => {
  try {
    const { code, name, unit, factory, min_stock, max_stock, unit_price, status } = req.body;
...
exports.updateMaterial = async (req, res) => {
  try {
    const { material_id } = req.params;
    const { code, name, unit, factory, min_stock, max_stock, current_stock, unit_price, status } = req.body;
    ...
    await material.update({
      ...
      factory: factory !== undefined ? factory : material.factory,
...
exports.deleteMaterial = async (req, res) => {
  try {
    ...
    // Check if material is referenced in other tables before deleting
